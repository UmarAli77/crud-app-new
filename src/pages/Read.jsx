    ...
    <div className="flex items-center justify-center w-full h-full">
        <div className="bg-emerald-500 rounded-lg w-[750px] h-auto flex shadow-xl">
            ...
            <div className="text-white pt-3 text-left">
                ...
                <h3 className="text-xl font-medium my-2 ml-10 capitalize">{data.price}</h3>
                <p className="ml-10 my-3">{data.description}</p>
                <p className="text-xl ml-10">Price: {data.price}.00</p>
                <div className="flex items-center relative">
                    <button className={`w-10 h-10 rounded-xl bg-${data.rating.count} border ml-10 my-3`}></button>
                    <p className="absolute right-[105px]">{ stars }</p>
                </div>
                <div className="flex items-center justify-around mt-1 mb-2 gap-5 ml-5">
                    <button className="border border-transparent text-white rounded-lg w-[150px] h-[40px] bg-gradient-to-r from-pink-500 to-orange-500 hover:from-teal-400 hover:to-blue-500"><Link to={`/update/${data.id}`}>Edit</Link></button>
